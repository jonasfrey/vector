...
function distanceBetweenPoints(x1, y1, x2, y2) {
  ...
  function f_b_collision__circle_and_polygonpoints(circle, polygonVertices) {
    let collision = false;
  
    // Check distance from circle center to polygon edges
    for (let i = 0; i < polygonVertices.length; i++) {
      let nextIndex = (i + 1) % polygonVertices.length;
      let edgeStart = polygonVertices[i];
      let edgeEnd = polygonVertices[nextIndex];
  
      let edgeLength = distanceBetweenPoints(edgeStart.x, edgeStart.y, edgeEnd.x, edgeEnd.y);
      let distanceToCircleCenter = Math.abs((edgeEnd.x - edgeStart.x) * (edgeStart.y - circle.centerY) - 
                                            (edgeStart.x - circle.centerX) * (edgeEnd.y - edgeStart.y)) / edgeLength;
  
      if (distanceToCircleCenter < circle.radius) {
        collision = true; // The circle and polygon might be colliding
        break;
      }
    }
  
    // Check if any vertex of polygon is inside the circle
    for (let vertex of polygonVertices) {
      if (isPointInsideCircle(vertex.x, vertex.y, circle)) {
        collision = true;
        break;
      }
    }
  
    // If no collision has been detected so far, check if the circle is inside the polygon
    if (!collision) {
      // Take a point on the circle (e.g., to the right of the center)
      let pointOnCircle = {
        x: circle.centerX + circle.radius * Math.cos(0), // 0 radians (to the right)
        y: circle.centerY + circle.radius * Math.sin(0)
      };
      collision = !isPointInsidePolygon(pointOnCircle.x, pointOnCircle.y, polygonVertices);
    }
...
let o_go__ship = new O_go(
    ...
    function(){
        ...
        let o_trn_speed = f_o_vec2(
            Math.cos(n_ang_rad+(n_tau/4))*2,
            Math.sin(n_ang_rad+(n_tau/4))*2,
        )//base speed
    ...
    function(){
        ...
        for(let o_go__shot of o_state?.a_o_go__shot){
            ...
            let b = f_b_collision__circle_and_polygonpoints(
                {
                    // we interpret a shot as a circle, sinec polygon overlapping detection would be 
                    // to computation intesive
                    radius: o_go__shot.n_radius,
                    centerX: o_go__shot.o_trn.x,
                    centerY: o_go__shot.o_trn.y
                }, 
                this.a_o_trn__shape_point
            )
            console.log(b)
